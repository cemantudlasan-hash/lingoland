...
export function useEnglishTugRoom() {
  ...
  const joinRoom = useCallback(
    async (
      ...
      const roomRef = doc(db, 'english_tug_rooms', code);
  ...
  const selectTeam = useCallback(
    async (roomCode: string, playerId: string, team: 'blue' | 'red') => {
      const db = getDb();
      await updateDoc(doc(db, 'english_tug_rooms', roomCode), {
...
// -------------------------------------------------------
export function useEnglishTugRoomListener(roomCode: string | null) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      doc(db, 'english_tug_rooms', roomCode),
      (docSnap) => {
        setLoading(false);
        if (docSnap.exists()) {
