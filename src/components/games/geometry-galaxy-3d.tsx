...
const SHAPE_DATA = [
  { name: "Cube", property: "A 3D solid with 6 equal square faces, 12 edges, and 8 vertices.", level: 1 },
  { name: "Sphere", property: "A perfectly round 3D object where every point on the surface is equal distance from the center.", level: 1 },
  { name: "Cone", property: "A solid with a circular base and a single vertex at the top.", level: 1 },
  { name: "Cylinder", property: "A solid with two parallel circular bases connected by a curved surface.", level: 1 },
  { name: "Pyramid", property: "A polyhedral object with a base and triangular faces that meet at a common apex.", level: 1 },
...
export function GeometryGalaxy3D({ slug, onToggleFullscreen }: { slug: string; onToggleFullscreen?: () => void }) {
  ...
  return (
    <Card className={cn(
      "w-full overflow-hidden bg-slate-950/90 backdrop-blur-md border-sky-500/20 shadow-2xl relative text-white transition-all duration-500",
        ...
        {gameState === 'finished' && (
          <div className="flex flex-col items-center gap-8 px-6 text-center">
            ...
            <div className="flex gap-4">
              <Button onClick={() => setGameState('selecting_difficulty')} size="lg" className="rounded-full bg-sky-500">NEW SECTOR</Button>
              <Button variant="outline" onClick={() => setGameState('idle')} size="lg" className="rounded-full border-sky-500">RETURN TO BASE</Button>
            </div>
          </div>
        )}
      </CardContent>
      ...
      <CardFooter className="bg-black/40 p-6 flex justify-between border-t border-sky-500/20">
        <Button variant="ghost" asChild className="text-sky-400"><Link href="/games">EXIT GALAXY</Link></Button>
        {gameState === 'playing' && (
          <div className="flex items-center gap-4 text-xs font-black text-sky-400">
            ...
            <div className="w-32 h-3 bg-white/10 rounded-full overflow-hidden">
              <motion.div className="h-full bg-sky-500" initial={{ width: 0 }} animate={{ width: `${(targetIndex / nodes.length) * 100}%` }} />
            </div>
          </div>
        )}
      </CardFooter>
