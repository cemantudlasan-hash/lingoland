...
export function VocabSnake() {
  ...
  // ─── Anti-Cheat Detection (Blur & Paste Prevention) ───
  useEffect(() => {
    if (gameState !== 'playing') return;
  ...
  const handleWordSubmit = (submittedWord?: string) => {
    ...
    // Rule 4: Theme Validation (if specific category selected)
    ...
    // Calculate score points: length * speed multiplier * difficulty
    ...
    return (
      <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-6">
        <motion.div
          ...
          className="text-center space-y-3 bg-gradient-to-r from-emerald-900/40 via-teal-900/60 to-cyan-900/40 p-8 rounded-3xl border border-emerald-500/30 shadow-2xl relative overflow-hidden backdrop-blur-xl"
        >
          <div className="absolute top-0 right-0 p-4 opacity-10 font-black text-8xl pointer-events-none">🐍</div>
          <Badge className="bg-emerald-500/20 text-emerald-300 border-emerald-500/40 px-3 py-1 font-bold text-xs uppercase tracking-widest">
            Multiplayer Word Chain Arena
          </Badge>
          <h1 className="text-4xl sm:text-5xl font-black text-white tracking-tight italic flex items-center justify-center gap-3">
            <span>🐍 Vocab Snake</span>
          </h1>
          <p className="text-emerald-200/80 text-sm max-w-xl mx-auto leading-relaxed font-medium">
          ...
          <div className="pt-2 flex justify-center items-center gap-2">
            <div className="bg-emerald-950/80 border border-emerald-500/30 px-4 py-2 rounded-2xl flex items-center gap-3">
              <span className="text-xs text-emerald-400 font-extrabold uppercase tracking-wider">Room Code:</span>
              <span className="text-lg font-black tracking-widest text-emerald-200">{roomCode}</span>
              <Button size="icon" variant="ghost" onClick={copyRoomCode} className="h-7 w-7 text-emerald-400 hover:text-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Card className="bg-zinc-950/60 border-zinc-800 backdrop-blur-xl p-6 rounded-3xl space-y-5">
            <h3 className="text-lg font-bold text-white flex items-center gap-2">
              <Zap className="h-5 w-5 text-emerald-400" /> Difficulty & Match Timer
            </h3>

            <div>
              <label className="text-xs text-zinc-400 font-bold uppercase tracking-wider block mb-2">Difficulty Level</label>
              <div className="grid grid-cols-3 gap-2">
                {(['easy', 'medium', 'hard'] as const).map(level => (
                  <button
                    ...
                    className={cn(
                      'py-3 px-2 rounded-xl text-xs font-bold capitalize transition-all border text-center',
                      difficulty === level
            ...
            <div>
              <label className="text-xs text-zinc-400 font-bold uppercase tracking-wider block mb-2">Total Match Duration</label>
              <div className="grid grid-cols-3 gap-2">
            ...
            <div>
              <label className="text-xs text-zinc-400 font-bold uppercase tracking-wider block mb-2">Total Players (Lobby / AI Bots)</label>
              <div className="flex items-center gap-3">
                ...
                />
                <span className="font-extrabold text-white text-lg bg-zinc-900 px-3 py-1 rounded-xl border border-zinc-800 min-w-[50px] text-center">
                  {playerCount}P
                </span>
              </div>
            </div>
          ...
          {/* Theme Categories */}
          <Card className="bg-zinc-950/60 border-zinc-800 backdrop-blur-xl p-6 rounded-3xl space-y-4">
            <h3 className="text-lg font-bold text-white flex items-center gap-2">
              <BookOpen className="h-5 w-5 text-teal-400" /> Category & Theme Mode
            </h3>
              ...
              {Object.entries(THEME_CATEGORIES).map(([key, item]) => (
                <button
                  ...
                  className={cn(
                    ...
                    selectedTheme === key
                      ? 'bg-gradient-to-r from-emerald-600/30 to-teal-600/30 border-emerald-500 text-white shadow-md'
                      : 'bg-zinc-900/50 border-zinc-800 text-zinc-400 hover:bg-zinc-800/80 hover:text-white'
                  )}
                >
                  <span className="text-2xl">{item.icon}</span>
                  <div>
                    <p className="text-xs font-bold leading-tight">{item.label}</p>
                    <p className="text-[10px] text-zinc-500 mt-0.5">{item.words.length} words</p>
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="pt-2 text-center">
          <Button
            ...
            className="w-full sm:w-auto px-12 h-14 bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-500 hover:from-emerald-400 hover:to-cyan-400 text-zinc-950 font-black text-lg rounded-2xl shadow-xl shadow-emerald-500/20 transform transition duration-300 hover:scale-105"
          >
            <Play className="h-5 w-5 fill-current mr-2" /> Start Vocab Snake Match
          </Button>
        </div>
      </div>
    );
    ...
    return (
      <div className="max-w-3xl mx-auto p-6 space-y-8 animate-in fade-in zoom-in-95 duration-500">
        <motion.div
          ...
          className="text-center space-y-4 bg-gradient-to-b from-amber-950/60 via-zinc-950/80 to-zinc-950 border border-amber-500/30 p-8 rounded-3xl shadow-2xl backdrop-blur-xl relative overflow-hidden"
        >
          ...
          <h2 className="text-4xl font-black text-white italic tracking-tight uppercase">Game Over!</h2>
          <p className="text-zinc-400 text-sm">Match timer expired. Here are the top word snake champions!</p>

          {/* Winner Banner */}
          {winner && (
            <div className="bg-gradient-to-r from-amber-500/20 via-amber-400/30 to-amber-500/20 border border-amber-400/40 p-4 rounded-2xl max-w-md mx-auto">
              <Crown className="h-8 w-8 text-amber-400 mx-auto mb-1 animate-pulse" />
              <p className="text-xs font-bold text-amber-300 uppercase tracking-widest">🏆 1st Place Winner</p>
              <h3 className="text-2xl font-black text-white">{winner.name}</h3>
              <p className="text-amber-200 font-extrabold text-lg mt-1">{winner.score} Pts · {winner.wordCount} Words</p>
            </div>
          )}
        </motion.div>

        {/* Podium / Top 3 Leaderboard */}
        <Card className="bg-zinc-950/70 border-zinc-800 p-6 rounded-3xl">
          <CardHeader className="p-0 mb-4">
            <CardTitle className="text-white text-lg font-bold flex items-center gap-2">
              <Medal className="h-5 w-5 text-amber-400" /> Final Scoreboard Standings
            </CardTitle>
          </CardHeader>
              ...
              <div
                ...
                className={cn(
                  ...
                  rank === 0
                    ...
                    : rank === 1
                    ? 'bg-zinc-800/40 border-zinc-400/30 text-zinc-200'
                    : rank === 2
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs opacity-75">{p.wordCount} words submitted</p>
                  </div>
                </div>
        ...
        <div className="flex justify-center gap-4">
          <Button
            ...
            className="px-8 h-12 bg-zinc-800 hover:bg-zinc-700 text-white font-bold rounded-xl"
          >
            <RotateCcw className="h-4 w-4 mr-2" /> Back to Lobby
          </Button>
          <Button
            onClick={startNewGame}
            className="px-8 h-12 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl"
          >
  ...
  // ─── Render: Main Active Match Screen ───
  return (
    <div className="max-w-6xl mx-auto p-3 sm:p-6 space-y-6">

      {/* Top Header Controls & Match Timer */}
      <div className="flex flex-wrap items-center justify-between gap-4 bg-zinc-950/80 border border-zinc-800 p-4 rounded-3xl backdrop-blur-xl">
        <div className="flex items-center gap-3">
          <span className="text-3xl">🐍</span>
          <div>
            <h2 className="text-lg font-black text-white italic tracking-tight">Vocab Snake Arena</h2>
            <div className="flex items-center gap-2 text-xs text-zinc-400 font-medium">
              <Badge variant="outline" className="text-emerald-400 border-emerald-500/30 capitalize">
                {difficulty} Mode
              </Badge>
              <span>•</span>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 bg-indigo-950/60 border border-indigo-500/30 px-4 py-2 rounded-2xl">
          ...
          >
            {soundEnabled ? <Volume2 className="h-5 w-5 text-emerald-400" /> : <VolumeX className="h-5 w-5 text-zinc-500" />}
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <Card className="bg-gradient-to-r from-emerald-950/40 via-zinc-950 to-teal-950/40 border-emerald-500/30 p-5 rounded-3xl relative overflow-hidden backdrop-blur-xl">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="relative">
                  <span className="text-4xl">{activePlayer?.avatar}</span>
                  <span className="absolute -bottom-1 -right-1 w-3.5 h-3.5 rounded-full bg-emerald-500 border-2 border-zinc-950 animate-ping" />
                </div>
                <div>
                  <p className="text-xs font-bold text-emerald-400 uppercase tracking-widest flex items-center gap-1">
                    🟢 Active Player Turn
                  </p>
                  <h3 className="text-xl font-black text-white">{activePlayer?.name}</h3>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 bg-zinc-900/80 px-4 py-2 rounded-2xl border border-zinc-800">
                ...
                <div>
                  <p className="text-[9px] text-zinc-400 font-extrabold uppercase">Turn Timer</p>
                  <p className={cn("text-lg font-black font-mono", turnTimeLeft <= 5 ? "text-rose-400 animate-bounce" : "text-amber-300")}>
                    {turnTimeLeft}s
                  </p>
                </div>
              </div>
          ...
          <Card className="bg-zinc-950/70 border-zinc-800 p-6 rounded-3xl space-y-4">
            <div className="flex justify-between items-center border-b border-zinc-800/80 pb-3">
              <h3 className="text-sm font-bold text-zinc-300 uppercase tracking-wider flex items-center gap-2">
                🐍 Connected Word Snake ({wordChain.length} Words)
              </h3>
              <Badge variant="secondary" className="bg-emerald-500/10 text-emerald-300 border-emerald-500/30 text-xs">
                Target Letter: <span className="text-base font-black text-emerald-400 ml-1">{currentTargetLetter || '?'}</span>
              </Badge>
            </div>

            {/* Scrollable Visual Snake Stream */}
            <div className="flex items-center gap-3 overflow-x-auto p-4 bg-zinc-900/60 rounded-2xl border border-zinc-800/80 min-h-[110px] scrollbar-thin scrollbar-thumb-zinc-700">
              {wordChain.map((item, idx) => (
                <React.Fragment key={item.id}>
                  <motion.div
                    ...
                    animate={{ scale: 1, opacity: 1 }}
                    className={cn(
                      'flex-shrink-0 p-3 rounded-2xl border text-center relative font-mono shadow-md',
                      idx === wordChain.length - 1
                        ? 'bg-gradient-to-br from-emerald-950/80 to-teal-900/80 border-emerald-400/80 text-white shadow-emerald-500/20'
                        : 'bg-zinc-900 border-zinc-800 text-zinc-300'
                    )}
                  >
                    <p className="text-xs text-zinc-400 font-bold mb-0.5 truncate max-w-[100px]">{item.playerName}</p>
                    <p className="text-base font-black tracking-wider uppercase">
                    ...
                    {item.points > 0 && (
                      <span className="text-[10px] font-bold text-emerald-400 bg-emerald-950/60 px-1.5 py-0.5 rounded-md mt-1 inline-block">
                        +{item.points} pts
                      </span>
                    )}
                  </motion.div>
                  {idx < wordChain.length - 1 && (
                    <ArrowRight className="h-4 w-4 text-emerald-500 flex-shrink-0 opacity-60" />
                  )}
                </React.Fragment>
              ))}
            </div>
          ...
          <Card className="bg-zinc-950/70 border-zinc-800 p-6 rounded-3xl space-y-4 relative overflow-hidden">
            {activePlayer?.isBot && (
              <div className="absolute inset-0 bg-zinc-950/80 backdrop-blur-sm z-20 flex items-center justify-center gap-3 text-zinc-300 font-bold">
            ...
            {!activePlayer?.isBot && activePlayer?.id !== 'p1' && (
              <div className="absolute inset-0 bg-zinc-950/85 backdrop-blur-sm z-20 flex flex-col items-center justify-center gap-2 text-zinc-400">
                <Lock className="h-6 w-6 text-amber-400" />
                <p className="font-extrabold text-white text-sm">🔒 Locked — Waiting for {activePlayer?.name}&apos;s turn</p>
            ...
            <div className="space-y-3">
              <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider flex items-center justify-between">
                ...
                <span className="text-[10px] text-rose-400 flex items-center gap-1">
                  <ShieldAlert className="h-3 w-3" /> Anti-Cheat Active (No Paste / Tab Switch)
                </span>
              </label>
              ...
              <div className="flex gap-3">
                <Input
                  ...
                  className="h-14 bg-zinc-900 border-zinc-800 text-white text-lg font-mono font-bold focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 rounded-2xl"
                  autoFocus
                />
                <Button
                  onClick={() => handleWordSubmit()}
                  className="h-14 px-6 bg-emerald-600 hover:bg-emerald-500 text-white font-black text-base rounded-2xl shadow-lg shadow-emerald-600/20"
                >
                  Submit
                </Button>
                <Button
                  ...
                  variant="outline"
                  className="h-14 px-4 border-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-900 rounded-2xl"
        ...
        <div className="lg:col-span-4 space-y-6">
          <Card className="bg-zinc-950/70 border-zinc-800 p-5 rounded-3xl space-y-4">
            <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center justify-between border-b border-zinc-800 pb-3">
              <span className="flex items-center gap-2">
                <Trophy className="h-4 w-4 text-amber-400" /> Live Match Leaderboard
              </span>
              <span className="text-[10px] text-emerald-400 font-extrabold">{players.length} Players</span>
            </h3>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      <span className="font-black text-xs text-zinc-500 w-5 text-center">
                      ...
                      <div>
                        <p className="font-bold text-xs text-white flex items-center gap-1.5">
                          {player.name}
                          {isActiveTurn && <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />}
                        </p>
                        <p className="text-[10px] text-zinc-400">{player.wordCount} words played</p>
                      </div>
                    </div>

                    <div className="text-right">
                      <p className="font-black text-sm text-emerald-400">{player.score} Pts</p>
                      {player.warningsCount > 0 && (
                        <p className="text-[9px] text-rose-400 font-bold flex items-center gap-0.5">
                          <AlertTriangle className="h-2.5 w-2.5" /> {player.warningsCount} Warn
                        </p>
                      )}
                    </div>
                  </div>
          ...
          {/* Quick Rules Box */}
          <Card className="bg-zinc-950/40 border-zinc-800 p-4 rounded-3xl text-xs space-y-2 text-zinc-400">
            ...
            <ul className="space-y-1 text-[11px] list-disc list-inside text-zinc-400 leading-relaxed">
